...
with Flow(
    "SMTR: Subsídio SPPO Recursos Bloqueio de Via - Captura/Tratamento",
    code_owners=["carolinagomes", "rafaelpinheiro"],
) as subsidio_sppo_bloqueio_via:
    ...
    rename_flow_run = rename_current_flow_run_now_time(
        prefix=subsidio_sppo_bloqueio_via.name + " ",
        now_time=timestamp,
    )
    ...
    subsidio_sppo_bloqueio_via.set_reference_tasks(
        [wait_materializacao, wait_recaptura, wait_captura]
    )

subsidio_sppo_bloqueio_via.storage = GCS(emd_constants.GCS_FLOWS_BUCKET.value)
subsidio_sppo_bloqueio_via.run_config = KubernetesRun(
    image=emd_constants.DOCKER_IMAGE.value,
    labels=[emd_constants.RJ_SMTR_DEV_AGENT_LABEL.value],
)

# Schedule
subsidio_sppo_bloqueio_via.schedule = every_day
