...
def get_data() -> pd.DataFrame:
    ...
    '''
    return bd.read_sql(query=query, billing_project_id="datario")
...
def format_message(dataframe: pd.DataFrame) -> List[str]:
    ...
    for row in range(len(dataframe)):
        ...
        if row < cluster:
            alert1 = alert1 + identification + " - " + \
                str(map_link(address, latlong)) + " - " + \
                thumbs_up + thumbs_up_emoji + "\n \n"
        elif cluster <= row < cluster*2:
            alert2 = alert2 + identification + " - " + \
                str(map_link(address, latlong)) + " - " + \
                thumbs_up + thumbs_up_emoji + "\n \n"
        elif cluster*2 <= row < cluster*3:
            alert3 = alert3 + identification + " - " + \
                str(map_link(address, latlong)) + " - " + \
                thumbs_up + thumbs_up_emoji + "\n \n"
        elif row >= cluster*3:
            alert4 = alert4 + identification + " - " + \
                str(map_link(address, latlong)) + " - " + \
                thumbs_up + thumbs_up_emoji + "\n \n"

    traffic_light_emoji = '\U0001F6A6'
    msg_alert = traffic_light_emoji + " CETRIO" + \
        "\n \nALERTA WAZE - Semáforo quebrado - atualizado em " + strftime("%Y-%m-%d %H:%M:%S") + \
        "\n \n"
