...
from pipelines.rj_sms.dump_ftp_sigtap.constants import constants as sigtap_constants
from pipelines.rj_sms.utils import download_ftp, create_folders, unzip_file
from pipelines.rj_sms.dump_ftp_sigtap.tasks import (
...
with Flow(
    name="SMS: Dump SIGTAP - Captura de dados SIGTAP", code_owners=["thiago"]
) as dump_sigtap:
    # Parameters
    # Parameters for GCP
    dataset_id = sigtap_constants.DATASET_ID.value
    # Parameters for CNES
    ftp_server = sigtap_constants.FTP_SERVER.value
    ftp_file_path = sigtap_constants.FTP_FILE_PATH.value
    base_file = sigtap_constants.BASE_FILE.value
    ...
    #    file_name=base_file,
    #)
    check_newest_file_version_task = {"file": "TabelaUnificada_202310_v2310101444.zip"}
...
dump_sigtap.storage = GCS(constants.GCS_FLOWS_BUCKET.value)
dump_sigtap.run_config = KubernetesRun(
...
dump_sigtap.schedule = every_sunday_at_six_am
