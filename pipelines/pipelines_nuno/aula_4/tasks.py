...
from pipelines.pipelines_nuno.aula_4.utils import limpa_numero
from pipelines.pipelines_nuno.aula_4.utils import cria_caminho
...
def escreve_resultado(meu_dataframe, timestamp_arquivo):
  ...
  meu_dataframe.to_csv('.\\Saida\\' + timestamp_arquivo + ".csv",
                       sep=";",
                       encoding='ansi')
...
def grava_estatisticas(meu_dataframe, timestamp_arquivo):
  ...
  porcentagem_paises = ((meu_dataframe["location.country"].value_counts() /
                         meu_dataframe["location.country"].count()) * 100).map(
                           "{:,.2f}%".format)
  porcentagem_generos = ((meu_dataframe["gender"].value_counts() /
                          meu_dataframe["gender"].count()) * 100).map(
                            "{:,.2f}%".format)
